...
class SessionModel
{
    ...
    {
        $sql = $this->conexion->query("INSERT INTO sesiones (id_usuario,token_sesion , fecha_inicio, fecha_expiracion) VALUES ('$id_usuario','$token','$fecha_hora_inicio','$fecha_hora_fin')");
        if ($sql) {
    ...
    public function verificar_sesion_si_activa($id_sesion, $token)
    {
        ...
        $b_sesion = $this->conexion->query("SELECT * FROM sesiones WHERE id_sesion='$id_sesion'");
        $datos_sesion = $b_sesion->fetch_object();

        $fecha_hora_fin_sesion = $datos_sesion->fecha_expiracion;
        ...
        if ((password_verify($datos_sesion->token_sesion, $token)) && ($hora_actual <= $fecha_hora_fin)) {
            ...
            $this->conexion->query("UPDATE sesiones SET fecha_expiracion='$nueva_fecha_hora_fin' WHERE id_sesion=$id_sesion");
            return 1;
        } else {
            return 0;
        }
